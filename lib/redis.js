...
export async function getPhoneHistory(phone) {
  try {
    ...
    console.error("Redis getPhoneHistory error:", e);
...
export async function updatePhoneHistory(phone, orderData, score) {
  try {
    ...
    console.error("Redis updatePhoneHistory error:", e);
  }
}
...
export async function incrementCancelCount(phone) {
  try {
    ...
    console.error("Redis incrementCancelCount error:", e);
  }
}
...
export async function updateAddressHistory(address, phone) {
  try {
    ...
    console.error("Redis updateAddressHistory error:", e);
  }
}
...
export async function saveOrderResult(orderId, result) {
  try {
    ...
    console.error("Redis saveOrderResult error:", e);
  }
}
...
export async function updateOrderStatus(orderId, status) {
  try {
    ...
    console.error("Redis updateOrderStatus error:", e);
  }
}
...
export async function updateStats(shopDomain, { isFraud, moneySaved, autoConfirmed, autoCancelled }) {
  try {
    ...
    console.error("Redis updateStats error:", e);
  }
}
...
export async function saveSettings(shopDomain, settings) {
  try {
    ...
    console.error("Redis saveSettings error:", e);
  }
}
...
export async function savePlan(shopDomain, plan) {
  try {
    await redis.set(`cod:plan:${shopDomain}`, JSON.stringify(plan));
  } catch (e) {}
}
...
export async function savePendingConfirmation(phone, orderData) {
  try {
    await redis.set(`cod:pending:${phone}`, JSON.stringify({ ...orderData, sentAt: Date.now() }), { ex: 1800 });
  } catch (e) {}
}
...
export async function deletePendingConfirmation(phone) {
  try {
    await redis.del(`cod:pending:${phone}`);
  } catch (e) {}
}
