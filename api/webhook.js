...
import {
  getPhoneHistory, updatePhoneHistory, getAddressHistory, updateAddressHistory,
  saveOrderResult, updateStats, incrementCancelCount,
} from "../lib/redis.js";
...
export default async function handler(req, res) {
  ...
  if (!verifyWebhookHmac(rawBody, hmac)) {
    console.error("Invalid HMAC for webhook from", shop);
  ...
  try {
    ...
    const { score, level, reasons } = calculateFraudScore(
      { total_price, phone, shipping_address, created_at, customer },
      phoneHistory,
      addressHistory
    );
    ...
    if (level === "safe") {
      await updateStats(shop, { isFraud: false, autoConfirmed: true });
    }
    ...
    if (level === "high_risk") {
      ...
      if (accessToken && settings.autoCancel !== false) {
        await cancelOrder(shop, accessToken, id);
      }
